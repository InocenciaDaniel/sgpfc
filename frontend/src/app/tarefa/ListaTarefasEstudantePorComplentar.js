...
export class ListaTarefasEstudante extends Component {
  ...
  fetchData = async () => {
    try {
      ...
      if (
        JSON.parse(usuario)?.fkUtilizador?.fkTipoConta?.designacao === "Estudante"
      ) {
        ...
        console.log("usuario", fkTurmaEstudanteMatriculadoResponse);
        this.setState({
    ...
    switch (value) {
      case "Novo":
        sortedTarefas = [...this.state.tarefas].filter(
          (product) => product.estadoTarefaEstudante === "Novo"
        );
        break;
      case "Em Revisão":
        sortedTarefas = [...this.state.tarefas].filter(
          (product) => product.estado === "Em Revisão"
        );
        break;
      case "Alterações Solicitadas":
        sortedTarefas = [...this.state.tarefas].filter(
          (product) =>
            product.estadoTarefaEstudante === "Alterações Solicitadas"
        );
        break;
      case "Concluído":
        sortedTarefas = [...this.state.tarefas].filter(
          (product) => product.estadoTarefaEstudante === "Concluído"
        );
        break;
    ...
    <div className="col-12 p-1">
      ...
      >
        <div className="flex-grow-1">
          ...
          >
            <div className=" mb-2">{data.designacaoTarefa}</div>
            ...
            <Dropdown
              ...
              options={[
                ...
                { label: "Em Revisão", value: "Em Revisão" },
                {
                  ...
                  value: "Alterações Solicitadas",
                },
                { label: "Concluído", value: "Concluído" },
              ]}
              optionLabel="label"
              placeholder="Filtrar Por Estado da Tarefa"
      ...
      <div>
        ...
        {usuarioLogado ? (
          <div>
            ...
            <DataView
              value={filteredItems || this.state.tarefas}
              layout={layout}
              paginator
              rows={12}
              sortOrder={sortOrder}
